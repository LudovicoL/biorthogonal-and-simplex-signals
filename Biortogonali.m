...
name_file=['Biortogonali_',num2str(data),'.txt'];
...
fprintf(1,'SEGNALI BIORTOGONALI\n');
...
for Ind_M=1:Lungh_M,                                % Scorro tutti i valori di M volta per volta.
    ...
    N=M(Ind_M)/2;                                   % Calcolo N in base al valore corrente di M.    
    ...
    for IT=1:Lungh_Eb_No_dB,                        % Scorro tutti i valori di Eb_No_dB volta per volta.
        ...
        % Genero una matrice A che contiene l'intero paniere dei segnali possibili (tutti i possibili casi):
        A=zeros(N,M(Ind_M));
        I_Col=1;
        for I_Rig=1:N,
            A(I_Rig, I_Col)=sqrt(Es);
            A(I_Rig, I_Col+1)=-sqrt(Es);
            I_Col=I_Col+2;
        end
        ...
        while (Errori<N_Errori)&&(Niter<MAX_Iter)
            ...
            % Creo il segnale trasmesso prendendolo dalla colonna puntata dall'indice di trasmissione nella matrice A:
            S_TX=A(:,Idx_TX);
            ...
            % Ottengo l'indice del valore massimo tra il prodotto del vettore trasporto del segnale ricevuto e la matrice A:
            [~, Idx_DEC]=max(S_RX'*A);
...
name_figure=['Biortogonali_',num2str(data),' Figure'];
...
save('variables_biortogonali', 'Eb_No_dB', 'Pe', 'M', 'data');
% Esporto tutte le varabili del programma:
name_all_variables=['Biortogonali_variables_',num2str(data)];
